...
tPosL findPosition(tList L, int d){
/**************************************************************************
...
**************************************************************************/
    ...
    while((p->next != LNULL) && strcmp(p->next->data, d) < 0) // Busca a posicion que ocupa o dato na lista. Sale do bucle cando chegado ao final da lista ou cando o id do produto que ocupa a seguinte posicion é maior alfabeticamente
        p = p->next;
...
bool insertItem(int d, tList *L){
    ...
    if (!createNode(&q))
        ...
        if (*L == LNULL)
            *L = q;
        else if (strcmp(d, (*L)->data) < 0){     // Caso no que se inserta na primeira posicion
...
int getItem(tPosL p, tList L){
    int empty={"", "", 0, 0, 0, .bidStack.top=NULL};
    if(L==LNULL)
...
void updateItem(int d,tPosL p,tList* L){
